...
# Install script for SOCKS5 Proxy on Amazon Linux 2023
echo "[+] Installing SOCKS5 Proxy on Amazon Linux 2023..."
...
# Open firewall if needed (Amazon Linux uses firewalld)
echo "[+] Opening firewall port 1337..."
if command -v firewall-cmd &> /dev/null; then
    firewall-cmd --permanent --add-port=1337/tcp
    firewall-cmd --reload
    echo "[+] Firewall port 1337 opened"
else
    # Fallback to iptables
    iptables -I INPUT -p tcp --dport 1337 -j ACCEPT 2>/dev/null || true
    echo "[+] iptables rule added for port 1337"
fi

# Install and enable neofetch from EPEL if wanted
...
echo "[+] Neofetch installed: $(which neofetch 2>/dev/null || echo 'not found')"
